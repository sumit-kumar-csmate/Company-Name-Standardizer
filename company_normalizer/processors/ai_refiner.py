...
Rules:
1. SPELLING & DIALECT ALIGNMENT: Fix typos (e.g. "Limted" → "Limited"). Unify dialects and standard trade abbreviations (e.g., "Tyre" → "Tire", "Impex" → "Import And Export", "Intl" → "International"). This should not be limited to certain words only. If a batch or group of companies are going to AI then it should check all the words in whole group for typos and make them correct and same if 2 more different companies does not exists.
...
3. GEOGRAPHY UNIFICATION: Do not change Vn to Vietnam but change Vietnam to "Vn". Similarly for other countries, change from Malaysia to "M", Bangladesh to "Bd", United Kingdom to "UK", United States or United States of America to "USA".
...
9. INTELLIGENT UNIFICATION & NOISE REMOVAL: The input is provided in groups of similar names. Examine each group carefully. If multiple names in a group represent the same company—even if they differ by NTN numbers, tax IDs, bank account details, addresses, or geographic locations—you MUST unify them to the exact same Refined name.
    *   Don't remove bank from Company names: If a company name is appended with a bank name (e.g., "... & Faysal Bank", "... & HBL"), KEEP the bank appendage.
