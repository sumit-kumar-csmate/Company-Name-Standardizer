...
8. STRICT LEGAL SUFFIX PRESERVATION: Do NOT change correct legal suffixes (Pvt Ltd, Limited, LLC, Inc, Corp, DP, etc.). DO NOT merge names that have fundamentally different legal entities/suffixes (e.g., "Adeline Chemical DP" vs "Adeline Chemical LLC"). Keep them as distinct outputs.
...
10. TYPO UNIFICATION (CRITICAL): Within each group, if two or more names differ by only 1–2 characters (such as a transposed pair of letters, a substituted letter, a missing letter, or an extra letter), treat them as spelling variants of the same company and unify ALL of them to ONE single authoritative spelling, UNLESS they have different legal suffixes (e.g., DP vs LLC) which means they are distinct entities. To choose the correct spelling: prefer the version that appears most frequently in the group, or the one that most closely matches a plausible real-world registered company name. Every entry in the group MUST output that exact same Refined name — never leave two near-identical spellings as separate outputs within the same group, unless they are different legal entities.
